...
public class alta_comanda extends JFrame {
	...
	private JButton btnCercarClient;
	private JButton btnAfegirClient;
	private String idEmpleat;

	public alta_comanda(String idEmpleat) {
		this.idEmpleat = idEmpleat;
		setModalExclusionType(ModalExclusionType.APPLICATION_EXCLUDE);
		setType(Type.UTILITY);
		setResizable(false);
		setTitle("Alta comanda");
		...
		btnGuardarComanda.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					idComanda=controlador.crearNovaComanda(txtCodiClient.getText(), txtDataComanda.getText(), "PENDENT", preuTotal );
				} catch (Exception e) {
		...
		Date date = new Date();
		txtDataComanda.setText(dateFormat.format(date));
		...
		btnCercarClient = new JButton("Clients");
		btnCercarClient.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				consultarClients();
			}
		});
		btnCercarClient.setBounds(102, 60, 74, 23);
		contentPane.add(btnCercarClient);
		
		btnAfegirClient = new JButton("New button");
		btnAfegirClient.setBounds(188, 56, 32, 23);
		contentPane.add(btnAfegirClient);
